...
import { generateMelody, getGenreMelodyStyles, type GeneratedMelody, type MelodyStyle } from '../utils/melodyGenerator';
import {
  playArrangement, stopArrangement, setTrackVolume, setTrackMute, setTrackSolo,
  renderArrangementToWav, type TrackId,
} from '../utils/mixer';

interface MixerStudioProps {
...
function MelodyLane({
  ...
  return (
    <div className="relative flex-1 bg-gray-900 rounded border border-gray-700/50 overflow-hidden" style={{ height: 56 }}>
      {activeStep !== null && (
      ...
      {melody.notes.map((note, i) => {
        ...
        const y = ((maxMidi - note.midi) / range) * 40 + 8;
        ...
        return (
          <div
            ...
            style={{ left: `${x}%`, width: `${Math.max(w, 0.5)}%`, top: `${y}%`, height: isActive ? 7 : 5 }}
          />
        );
      })}
    </div>
...
export function MixerStudio({ progression, rhythm, rootKey, scaleType, bpm, genreId, chordSynthId, loop }: MixerStudioProps) {
  ...
  }, [progression, rhythm, bassMelody, leadMelody, chordSynthId, bpm, loop]);
  ...
  }, [progression, rhythm, bassMelody, leadMelody, chordSynthId, bpm, tracks, rootKey, scaleType]);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-white">Full Mix</h2>
          <p className="text-xs text-gray-500">Chords, bass and lead playing together ({rootKey} {scaleType} · {bpm} BPM)</p>
        </div>
      ...
      <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-900/50 border border-gray-800">
        ...
        <div className="relative flex-1 bg-gray-900 rounded border border-gray-700/50 overflow-hidden flex" style={{ height: 56 }}>
          {activeStep !== null && (
            <div className="absolute top-0 bottom-0 w-px bg-white/50 z-10" style={{ left: `${(activeStep / totalSteps) * 100}%` }} />
          )}
